...
(async () => {
  try {
    ...
    stream.write(
      "| Component | CSS Variables | Documented CSS Variables |\n| ----- | ----- | ----- |\n",
    );
    ...
    for (const component of components) {
      const stylesheet = await readFile(
        resolve(componentsPath, component, `${component}.scss`),
        "utf8",
      );
      // match css vars, make unique, sort
      const cssVars = [
        ...new Set(stylesheet.match(/--calcite-[^\]\)\}\;\:\,\s]*/g)),
      ].sort();
      // match the css vars documented at the top of the stylesheet in the comments
      const cssVarsDocd = [
        ...new Set(
          stylesheet.match(/(?<=\* \@prop )--calcite-[^\]\)\}\;\:\,\s]*/g),
        ),
      ].sort();

      stream.write(
        `| ${component} | ${cssVars.join("<br />")} | ${cssVarsDocd.join("<br />")} |\n`,
      );
    }
  } catch (err) {
    console.error(err);
